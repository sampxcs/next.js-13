...
export default async function Page () {
  ...
  const posts = postsContents.slice(index, index + 10).map((post: any) => {
    return {
      ...post,
  ...
  return (
    <ul className='flex flex-col gap-6'>
      {posts.map(({ id, title, body, user, gif } :any) => (
        <li key={id}>
          <Link href={`/post/${id}/${gif.id}`} className='flex rounded-xl p-6 border-[1px] transition-colors border-transparent hover:bg-opacity-5 dark:hover:bg-opacity-20 hover:bg-neutral-800 hover:border-neutral-200 dark:hover:border-neutral-600'>
